...
Test reconciliation consumes Homeboy's `homeboy/test-outcomes/v1` and `homeboy/test-inventory/v1` sidecars named `<command-stem>.test-outcomes.json` and `<command-stem>.test-inventory.json`. Failed candidate and baseline Test phases require complete, provenance-matched evidence; unavailable evidence is a blocking `no_comparable_evidence` result. This consumer contract depends on Extra-Chill/homeboy#11121's forthcoming producer change.
