...
autofix_cached_diff_line_count() {
...
autofix_max_allowed_line_count() {
  local total_fixes per_fix absolute proportional
  total_fixes="$(autofix_total_report_fixes)"
  per_fix="${AUTOFIX_MAX_LINES_PER_FIX:-50}"
  absolute="${AUTOFIX_MAX_CHANGED_LINES:-250}"
  ...
  proportional=$((total_fixes * per_fix))
  if [ "${proportional}" -lt "${absolute}" ]; then
    printf '%s\n' "${proportional}"
  else
    printf '%s\n' "${absolute}"
  fi
...
autofix_max_allowed_file_count() {
  local total_fixes per_fix absolute proportional
  total_fixes="$(autofix_total_report_fixes)"
  per_fix="${AUTOFIX_MAX_FILES_PER_FIX:-3}"
  absolute="${AUTOFIX_MAX_CHANGED_FILES:-10}"
...
autofix_diff_safety_status() {
  local file_count line_count max_files max_lines total_fixes

  file_count="$(git diff --cached --name-only | sed '/^[[:space:]]*$/d' | wc -l | xargs)"
  line_count="$(autofix_cached_diff_line_count)"
  max_files="$(autofix_max_allowed_file_count)"
  max_lines="$(autofix_max_allowed_line_count)"
  total_fixes="$(autofix_total_report_fixes)"

  if [ "${file_count}" -gt "${max_files}" ]; then
    printf 'too-many-files: changed %s files for %s reported fixes; limit is %s files\n' "${file_count}" "${total_fixes}" "${max_files}"
    return 1
  fi

  if [ "${line_count}" -gt "${max_lines}" ]; then
    printf 'too-many-lines: changed %s lines for %s reported fixes; limit is %s lines\n' "${line_count}" "${total_fixes}" "${max_lines}"
    return 1
  fi

  printf 'safe: changed %s files and %s lines for %s reported fixes\n' "${file_count}" "${line_count}" "${total_fixes}"
}
