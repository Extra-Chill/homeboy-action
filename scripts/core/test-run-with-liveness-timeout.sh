...
set +e
HOMEBOY_ACTION_EXECUTION_TIMEOUT_SECONDS=1 bash "${RUNNER}" "stalled command" bash -c 'sleep 3' >"${TMP_DIR}/timeout.log" 2>&1
exit_code=$?
set -e
...
if ! grep -q 'stalled command exceeded its 1s execution timeout' "${TMP_DIR}/timeout.log"; then
...
if [ "${wrapper_count}" -ne 1 ]; then
  printf 'FAIL: action must bound command phases, got %s wrappers\n' "${wrapper_count}"
  exit 1
fi
printf 'PASS: action applies bounded execution to command phases\n'
